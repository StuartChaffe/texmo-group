...
<?php if ( $image ) { ?><?php if ( $image['banner_image_overlay'] ) { ?><div class="banner--overlay" style="background-color: rgba(0,0,0,0.<?php echo $image['banner_image_overlay'] ?>);">&nbsp;</div><?php } ?><?php } ?>
	...
	<?php if ( $content ) { ?>
		...
		<?php if ( $video['banner_video_id'] ) { ?>
			<a class="btn-video" href="https://player.vimeo.com/video/<?php echo $video['banner_video_id'] ?>?autoplay=1&loop=1&autopause=0" data-featherlight="iframe" allow="autoplay; fullscreen" allowfullscreen ><svg class="icon icon--play"><use xlink:href="#play"></use></svg> <?php echo $video['banner_video_text']; ?></a>
		<?php } ?>
		</div>
	<?php } ?>
